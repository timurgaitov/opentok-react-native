...
public class AndroidVideoCapturer implements Camera.PreviewCallback {
    ...
    Runnable newFrame = new Runnable() {
        @Override
        public void run() {
            if (mCustomVideoCapturer.getIsCaptureRunning()) {
                if (frame == null) {
                ...
                mCustomVideoCapturer.provideIntArrayFrame(frame, BaseVideoCapturer.ARGB, width, height, 0, false);
                handler.postDelayed(newFrame, 1000 / fps);
            }
        }
    };

    public AndroidVideoCapturer(Context context, Publisher.CameraCaptureResolution resolution, Publisher.CameraCaptureFrameRate fps, CustomVideoCapturer customVideoCapturer) {
    ...
    public synchronized int startCapture(CameraType cameraType) {
        int cameraIndex = getCameraIndexUsingFront(cameraType);
        try {
    ...
    public BaseVideoCapturer.CaptureSettings getCaptureSettings() {
        ...
        if (camera != null) {
            settings = new BaseVideoCapturer.CaptureSettings();
            configureCaptureSize(resolution.width, resolution.height);
            ...
            settings.format = BaseVideoCapturer.ARGB;
        }
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        if (mCustomVideoCapturer.getIsCaptureRunning()) {
            if (data.length == expectedFrameSize) {
                ...
                mCustomVideoCapturer.provideByteArrayFrame(data, BaseVideoCapturer.NV21, captureWidth, captureHeight, currentRotation, isFrontCamera());
    ...
    public synchronized void swapCamera(CameraType cameraType, boolean isCaptureStarted) {
        if (camera != null) {
            stopCapture();
        }
        ...
        if (isCaptureStarted) {
            if (-1 != Build.MODEL.toLowerCase().indexOf("samsung")) {
                ...
                forceCameraRelease(cameraType);
            }

            startCapture(cameraType);
        }
    }
    ...
    private static int getCameraIndexUsingFront(CameraType cameraType) {
        boolean front = cameraType == CameraType.AndroidFront;
    ...
    private boolean forceCameraRelease(CameraType cameraType) {
        int cameraIndex = getCameraIndexUsingFront(cameraType);
        Camera camera = null;
        try {
            camera = Camera.open(cameraIndex);
