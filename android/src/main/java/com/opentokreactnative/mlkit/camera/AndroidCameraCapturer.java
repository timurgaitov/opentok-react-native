...
package com.opentokreactnative.mlkit.camera;
 ...
 */
public class AndroidCameraCapturer {
  ...
  private static final String TAG = "OTRN:AndroidCameraCapturer";
  ...
  private ProcessorFrameListener frameListener;
  ...
  public AndroidCameraCapturer(Activity activity, com.opentokreactnative.mlkit.processors.FrameProcessingRunnable runnable) {
  ...
  public synchronized AndroidCameraCapturer start() throws IOException {
    if (camera != null) {
      return this;
    }
   ...
   */
  @RequiresPermission(Manifest.permission.CAMERA)
  public synchronized AndroidCameraCapturer start(SurfaceHolder surfaceHolder) throws IOException {
    if (camera != null) {
      return this;
    }
  ...
  public synchronized void setFacing(CameraType type) {
    if (type == CameraType.External) {
      throw new IllegalArgumentException("Invalid camera: " + type);
    }
    this.facing = type == CameraType.AndroidFront ? CAMERA_FACING_FRONT : CAMERA_FACING_BACK;
  }
  ...
  public BaseVideoCapturer.CaptureSettings getCaptureSettings() {
    ...
    int format =  isFrameProcessorActive ? BaseVideoCapturer.ARGB : BaseVideoCapturer.NV21;

    int width = previewSize != null ? previewSize.getWidth() : 0;
    int height = previewSize != null ? previewSize.getHeight() : 0;
  ...
  private Camera createCamera() throws IOException {
    ...
    Log.v(TAG, "Camera preview fps: " + previewFpsRange[0] + " " + previewFpsRange[1]);
  ...
  public void onFrameProcessorEnabled(boolean enabled) {
  ...
  private class CameraPreviewCallback implements Camera.PreviewCallback {
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
      processingRunnable.setPreviewSize(previewSize.getWidth(), previewSize.getHeight());
      processingRunnable.setRotation(rotationDegrees);
      processingRunnable.setNextFrame(data, camera, bytesToByteBuffer);
      if (!isFrameProcessorActive && frameListener != null) {
          frameListener.onFrame(data, previewSize.getWidth(), previewSize.getHeight(), rotationDegrees);
      }
    }
  }

  public void setFrameListener(ProcessorFrameListener listener) {
